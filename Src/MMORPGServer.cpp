	...
	while (true) {
		...
		if (res == false || bytesTrans == 0) {
			if (session) {
				...
				GameService::GetInstance().LeaveGame(id);
				session->Disconnect();
				session->ReleaseRef();
			}
			continue;
		...
		if (ioData->ioType == IO_TYPE::RECV) {
			...
			while (true)
			{
				...
				newJob->kind = JobKind::CientPacket;
...
void LogicTreadMain()
{
	while (true)
	{
		...
		switch (job->kind)
		{
		case JobKind::CientPacket:
			ClientPacketHandler::HandlePacket(session, (BYTE*)job->rawData.data(), job->len, job->protocolId);
			break;
		case JobKind::SendPacket:
			SendPacketHandler::HandlePacket(session, (BYTE*)job->rawData.data(), job->len, job->protocolId);
			break;
		case JobKind::DbResult:
